...
public class MapExtension {
	...
	public static <K, O, N> Map<K, N> mapValues(Map<K, O> map, BiFunction<K, O, N> mapper) {
		return mapValues(map, mapper, new HashMap<>(), false);
	}
	
	public static <K, O, N> Map<K, N> mapValues(Map<K, O> map, BiFunction<K, O, N> mapper, Map<K, N> newMap) {
		return mapValues(map, mapper, newMap, false);
	}

	public static <K, O, N> Map<K, N> mapValues(Map<K, O> map, BiFunction<K, O, N> mapper, boolean makeImmutable) {
		return mapValues(map, mapper, new HashMap<>(), makeImmutable);
	}

	public static <K, O, N> Map<K, N> mapValues(Map<K, O> map, BiFunction<K, O, N> mapper, Map<K, N> newMap, boolean makeImmutable) {
		for (Map.Entry<K, O> entry : map.entrySet()) {
